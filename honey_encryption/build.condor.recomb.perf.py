...
# your Executable
Executable      = $(InputDir)/perf_test.py
...
def build_condor_file():
	...
	# recomb
	for i in range(1, 23):
		for seqIdx in range(DatasetSize):
			comment='Condor process : %s, chromosome: %s, seqIdx: %s' %( condor_process, i, seqIdx)
			outputfile = """$(OutputDir)/big_recomb_perf_chr%s_%s_CEU.txt.txt""" % (i, seqIdx)
			arguments = """cluster_encode perfFileName=%s seqIdx=%s modelName=RecombGenoGuard genotypeFileName=$(InputDir)/hapmap/chr%s/big_genotypes_chr%s_CEU.txt haplotypeFileName=$(InputDir)/hapmap/chr%s/big_CEU.chr%s.hap geneticDistFileName=$(InputDir)/hapmap/chr%s/big_genetic_map_chr%s_combined_b36.txt""" % (outputfile, seqIdx, i, i, i, i, i, i)
