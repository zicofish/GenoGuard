...
def build_condor_file():
	...
	for i in range(1, 23):
		for j in range(10):
			...
			outputfile = """$(OutputDir)/big_condProb0_chr%s_bin_seq_%s_CEU.txt.txt""" % (i, j)
			arguments = """clusterCalcModelBins binSeqFileName=%s binIdx=%s modelName=DirectCondProbGenoGuard genotypeFileName=$(InputDir)/hapmap/chr%s/big_genotypes_chr%s_CEU.txt probFileName=$(InputDir)/hapmap/chr%s/big_condProb0_chr%s_CEU_ref.txt intSeqsFileName=$(InputDir)/hapmap/chr%s/big_int_seqs_chr%s_CEU.pickle order=0""" % (outputfile, j, i, i, i, i, i, i)
